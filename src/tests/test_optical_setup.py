...
def test_pbs():
    assert pbs_mueller_matrix() * horizontal_polarization == Matrix.vstack(no_beam, horizontal_polarization)
    assert pbs_mueller_matrix() * vertical_polarization == Matrix.vstack(vertical_polarization, no_beam)
    assert pbs_mueller_matrix() * left_circular_polarization == Matrix.vstack(vertical_polarization / 2, horizontal_polarization / 2)
    assert pbs_mueller_matrix() * right_circular_polarization == Matrix.vstack(vertical_polarization / 2, horizontal_polarization / 2)
